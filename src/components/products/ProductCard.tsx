...
export function ProductCard({ product, onStatusChange }: ProductCardProps) {
  ...
  return (
    <div
      className={`group relative bg-white rounded-3xl border transition-all duration-300 flex flex-col overflow-hidden ${
        !isActiveState
    ...
    >
      ...
      >
        ...
        <button
          ...
          className={`absolute top-3 right-3 w-8 h-8 rounded-full bg-white/90 backdrop-blur-xs flex items-center justify-center border border-[#EFE6DA] shadow-2xs transition-all active:scale-90 z-10 ${
            isWishlisted ? 'text-[#F27A8A] bg-[#FDE8EB]' : 'text-gray-400 hover:text-[#F27A8A]'
          }`}
          aria-label="Save to wishlist"
        >
          <Heart className={`w-4 h-4 ${isWishlisted ? 'fill-[#F27A8A]' : ''}`} />
        </button>

        {/* Discount Badge */}
        {product.discount > 0 && isActiveState && (
          <span className="absolute top-3 left-3 px-2.5 py-0.5 text-[10px] sm:text-[11px] font-extrabold bg-[#F27A8A] text-white rounded-full shadow-cute-pink">
        ...
        {!isActiveState && (
          <div className="absolute inset-0 bg-[#193653]/60 backdrop-blur-2xs flex items-center justify-center">
      ...
      <div className="p-3.5 sm:p-4 flex-1 flex flex-col justify-between gap-2.5">
        <Link href={`/products/${product.id}`} className="space-y-1 block">
          ...
          <h3 className="text-xs sm:text-sm font-bold text-[#193653] group-hover:text-[#F27A8A] transition-colors line-clamp-1 leading-snug">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="pt-2 border-t border-[#EFE6DA]/70 flex items-center justify-between gap-2">
          ...
          {isAdmin ? (
            <div className="flex items-center gap-1">
              <button
                ...
                className={`p-1.5 rounded-xl text-xs font-bold border transition-all ${
                  isActiveState
                    ? 'bg-[#FEF9E8] text-[#E0B538] border-[#F6D77A]'
                    : 'bg-[#EFF7E9] text-[#729c50] border-[#A8C98B]'
                }`}
              >
              ...
              <Link
                ...
                className="p-1.5 rounded-xl bg-[#EBF8FC] text-[#3599b8] border border-[#8FD3E8]/40 text-xs font-bold transition-all"
                title="Edit Product"
              >
            ...
            <button
              ...
              className={`p-2 rounded-full text-xs font-bold flex items-center justify-center transition-all shadow-cute-pink active:scale-90 ${
                added
